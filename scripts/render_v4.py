...
for i, (row, voice) in enumerate(zip(manifest["segments"], timing, strict=True)):
    duration = max(reading_durations[i], math.ceil((voice["seconds"] + .35) * 30) / 30)
    ...
    d.multiline_text((82, 248), row["title"], font=font(82, True), fill="#f4f6fc", spacing=12)
    d.multiline_text((88, 494), row["body"], font=font(36), fill="#c8d2e4", spacing=14)
