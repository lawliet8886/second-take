...
class ConversationForkFlowTest {
    ...
    private fun capture(name:String,waitForIdle:Boolean=true){if(waitForIdle)composeRule.waitForIdle();Thread.sleep(180);val instrumentation=InstrumentationRegistry.getInstrumentation();val screenshot=instrumentation.uiAutomation.takeScreenshot();if(screenshot!=null){val dir=File(instrumentation.targetContext.getExternalFilesDir(null),"v071-gold").apply{mkdirs()};FileOutputStream(File(dir,"$name.png")).use{screenshot.compress(Bitmap.CompressFormat.PNG,100,it)}};if(InstrumentationRegistry.getArguments().getString("recordingMode")=="true")Thread.sleep(3_000)}
